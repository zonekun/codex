...
Usage:
  # トップレベル flag 追加
  PYTHONUTF8=1 python scripts/monthly_bc_repair/apply_adapter_patch.py --ticker 8218 \
      --patch '{"year_month_from_submission_minus_1": true}'

  # fields[*] の bc_ignore/yoy_offset/bc_key 等を特定 key 対象に変更
  PYTHONUTF8=1 python scripts/monthly_bc_repair/apply_adapter_patch.py --ticker 7532 \
...
def main() -> int:
    ...
    args = p.parse_args()
    ticker = args.ticker
    adapter_path = ROOT / f"meta/monthly/{ticker}_extract_adapter.json"
