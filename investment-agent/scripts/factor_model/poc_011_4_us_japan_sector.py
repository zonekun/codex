"""PoC: 011-4 US Sector ETF -> Japan Sector ETF lead-lag (subspace-regularised PCA).

Paper: SIG-FIN-036-13
Knowledge: docs/knowledges/analysis/011-4_us_japan_sector_leadlag.md

Structure:
    information source A = US Select Sector SPDR ETF 11 (Close-to-Close)
    investment target  B = Japan TOPIX-17 sector ETF 17 (Open-to-Close, next business day)

    Signal at US close (JST 06:00) -> Japan open order (JST 09:00) -> exit at Japan close (15:00)
    No 1-day lag problem: 3-4 hour gap makes this executable for retail investors.

Pipeline:
    1. Fetch US sector ETFs from yfinance (cache locally)
    2. Fetch Japan TOPIX-17 ETFs from BigQuery STOCK_PRICE_JQUANTS
    3. Align to common business days (US close day t -> JP next biz day t+1)
    4. Build V0 (K0=3): global, country-spread, cyclical/defensive
    5. Compute C0 from warm-up period (2020-2022)
    6. Rolling L=60 window: C_t^reg = 0.1*C_t + 0.9*C0
    7. Top-K=3 eigenvectors -> B_t = V_JP @ V_US.T
    8. Signal: z_hat_JP = B_t @ z_US,t -> long top-N sectors
    9. Evaluate: top-3 / top-5 / top-6, daily / weekly rebalance
   10. Baselines: FLAT, simple US momentum, lambda=0 PCA

Outputs:
    C:\\tmp\\poc_011_4_us_japan_sector\\metrics_summary.csv
    C:\\tmp\\poc_011_4_us_japan_sector\\returns_daily.csv
    C:\\tmp\\poc_011_4_us_japan_sector\\returns_weekly.csv
    C:\\tmp\\poc_011_4_us_japan_sector\\signal_sample.csv
    C:\\tmp\\poc_011_4_us_japan_sector\\us_sector_prices.csv
"""
from __future__ import annotations

import sys
import warnings
from datetime import datetime
from pathlib import Path
from zoneinfo import ZoneInfo

import numpy as np
import pandas as pd
from google.cloud import bigquery
from google.oauth2 import service_account

warnings.filterwarnings("ignore", category=FutureWarning)

# ---------------------------------------------------------------------------
# Config
# ---------------------------------------------------------------------------
JST = ZoneInfo("Asia/Tokyo")

PROJECT = "gmailpj-357912"
KEY_PATH = Path(r"C:\gdrive\claude\investment-agent\keys\gcp-service-account.json")

CACHE_DIR = Path(r"C:\tmp\poc_011_4_us_japan_sector")
OUTPUT_DIR = Path(r"C:\tmp\poc_011_4_us_japan_sector")
CACHE_DIR.mkdir(parents=True, exist_ok=True)

# US sector ETFs
US_TICKERS = ["XLB", "XLC", "XLE", "XLF", "XLI", "XLK", "XLP", "XLRE", "XLU", "XLV", "XLY"]
N_US = len(US_TICKERS)  # 11

# Japan TOPIX-17 sector ETFs
JP_TICKERS = [str(t) for t in range(1617, 1634)]  # 1617..1633
N_JP = len(JP_TICKERS)  # 17

N_TOTAL = N_US + N_JP  # 28

# Cyclical / Defensive classification (from paper)
US_CYCLICAL = {"XLB", "XLE", "XLF", "XLRE"}
US_DEFENSIVE = {"XLK", "XLP", "XLU", "XLV"}
JP_CYCLICAL = {"1618", "1625", "1629", "1631"}
JP_DEFENSIVE = {"1617", "1621", "1627", "1630"}

# Strategy params
WARMUP_START = pd.Timestamp("2020-01-01")
WARMUP_END = pd.Timestamp("2022-12-31")
OOS_START = pd.Timestamp("2023-01-01")
OOS_END = pd.Timestamp("2024-12-31")
WINDOW_L = 60
K_EIG = 3
LAMBDA_REG = 0.9  # C^reg = (1-lambda)*C_t + lambda*C0

# Cost / tax
ONEWAY_COST_BPS = 1  # ETF large-cap, high liquidity
TAX_RATE = 0.20315

# Top-N candidates for long-only
TOP_N_LIST = [3, 5, 6]

FORCE_RELOAD = False

import structlog

log = structlog.get_logger()


# ---------------------------------------------------------------------------
# Data fetching
# ---------------------------------------------------------------------------
def fetch_us_data() -> pd.DataFrame:
    """Fetch US sector ETF daily prices from yfinance, with CSV cache."""
    cache_path = CACHE_DIR / "us_sector_prices.csv"
    if not FORCE_RELOAD and cache_path.exists():
        log.info("US data from cache", path=str(cache_path))
        df = pd.read_csv(cache_path, parse_dates=["Date"], encoding="utf-8")
        return df

    import yfinance as yf

    log.info("Downloading US sector ETFs from yfinance", tickers=US_TICKERS)
    raw = yf.download(US_TICKERS, start="2019-12-01", end="2025-01-01", auto_adjust=True)

    # Extract Close and Open prices
    close = raw["Close"][US_TICKERS].copy()
    open_ = raw["Open"][US_TICKERS].copy()

    # Flatten to long format for cache
    close_long = close.stack().reset_index()
    close_long.columns = ["Date", "Ticker", "Close"]
    open_long = open_.stack().reset_index()
    open_long.columns = ["Date", "Ticker", "Open"]

    df = close_long.merge(open_long, on=["Date", "Ticker"], how="outer")
    df["Date"] = pd.to_datetime(df["Date"]).dt.tz_localize(None)
    df.to_csv(cache_path, index=False, encoding="utf-8")
    log.info("US data cached", rows=len(df), path=str(cache_path))
    return df


def fetch_jp_data() -> pd.DataFrame:
    """Fetch Japan TOPIX-17 ETF daily prices from BigQuery, with parquet cache."""
    cache_path = CACHE_DIR / "jp_sector_prices.parquet"
    if not FORCE_RELOAD and cache_path.exists():
        log.info("JP data from cache", path=str(cache_path))
        return pd.read_parquet(cache_path)

    creds = service_account.Credentials.from_service_account_file(str(KEY_PATH))
    bq = bigquery.Client(credentials=creds, project=PROJECT)

    ticker_list = ",".join(f"'{t}'" for t in JP_TICKERS)
    query = f"""
        SELECT TICKER, DATE, ADJ_OPEN, ADJ_CLOSE
        FROM `{PROJECT}.STOCK.STOCK_PRICE_JQUANTS`
        WHERE TICKER IN ({ticker_list})
          AND DATE BETWEEN '2019-12-01' AND '2024-12-31'
        ORDER BY TICKER, DATE
    """
    log.info("Querying BQ for JP sector ETFs")
    job = bq.query(query)
    df = job.to_dataframe()
    bytes_processed = job.total_bytes_processed or 0
    log.info("BQ query done", rows=len(df), gb=f"{bytes_processed / 1e9:.3f}")
    df["DATE"] = pd.to_datetime(df["DATE"])
    df.to_parquet(cache_path, index=False)

    # Log BQ cost
    with (CACHE_DIR / "bq_cost.log").open("a", encoding="utf-8") as f:
        ts = datetime.now(tz=JST).strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"{ts}\tjp_sector_prices\t{bytes_processed}\t{len(df)}\n")

    return df


# ---------------------------------------------------------------------------
# Return construction
# ---------------------------------------------------------------------------
def build_us_returns(df_us: pd.DataFrame) -> pd.DataFrame:
    """Build US sector close-to-close log returns (wide: date x ticker)."""
    df = df_us.copy()
    df = df.sort_values(["Ticker", "Date"]).drop_duplicates(["Ticker", "Date"], keep="last")
    pivot_close = df.pivot(index="Date", columns="Ticker", values="Close").sort_index()

    # Forward-fill for minor gaps, then compute log returns
    pivot_close = pivot_close.ffill(limit=3)
    log_ret = np.log(pivot_close / pivot_close.shift(1))
    # Reorder columns to match US_TICKERS order
    log_ret = log_ret[US_TICKERS]
    return log_ret


def build_jp_returns(df_jp: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Build Japan sector open-to-close and close-to-close log returns.

    Returns:
        oc: open-to-close log returns (wide: date x ticker)
        c2c: close-to-close log returns (wide: date x ticker)
    """
    df = df_jp.copy()
    df = df.sort_values(["TICKER", "DATE"]).drop_duplicates(["TICKER", "DATE"], keep="last")

    # Pivot
    pivot_open = df.pivot(index="DATE", columns="TICKER", values="ADJ_OPEN").sort_index()
    pivot_close = df.pivot(index="DATE", columns="TICKER", values="ADJ_CLOSE").sort_index()

    # Forward-fill missing (low volume ETFs sometimes have no trades)
    pivot_open = pivot_open.ffill(limit=5)
    pivot_close = pivot_close.ffill(limit=5)

    # Reorder columns
    pivot_open = pivot_open[JP_TICKERS]
    pivot_close = pivot_close[JP_TICKERS]

    oc = np.log(pivot_close / pivot_open)
    c2c = np.log(pivot_close / pivot_close.shift(1))
    return oc, c2c


def align_common_dates(
    us_c2c: pd.DataFrame,
    jp_oc: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.Index]:
    """Align US day t with JP day t+1 on common business day pairs.

    For each US trading day t, find the next JP trading day as t+1.
    Returns aligned DataFrames where row i of us_aligned is US day t,
    and row i of jp_aligned is JP day t+1 (the next JP trading day after US day t).
    """
    us_dates = us_c2c.dropna(how="all").index.sort_values()
    jp_dates = jp_oc.dropna(how="all").index.sort_values()

    # For each US date, find the next JP date
    jp_dates_set = set(jp_dates)
    jp_dates_sorted = sorted(jp_dates)

    pairs = []  # (us_date, jp_next_date)
    jp_idx = 0
    for us_d in us_dates:
        # Find next JP date >= us_d + 1 day (i.e., strictly after US date)
        # US close = JST 6:00 of us_d+1 (US calendar), but the actual date
        # in US calendar is us_d. The next JP trading day is typically us_d+1
        # in JP calendar.
        target = us_d + pd.Timedelta(days=1)
        while jp_idx < len(jp_dates_sorted) and jp_dates_sorted[jp_idx] < target:
            jp_idx += 1
        if jp_idx >= len(jp_dates_sorted):
            break
        jp_d = jp_dates_sorted[jp_idx]
        # Don't pair if the gap is too large (> 7 calendar days, e.g. long holidays)
        if (jp_d - us_d).days <= 7:
            pairs.append((us_d, jp_d))
        # Reset jp_idx for next US date (allow reuse since US dates are sorted)
        # Actually we need to keep searching forward, but US dates are also sorted
        # so jp_idx can stay where it is. But we might miss if JP date is reused.
        # Let's just search from current position.

    if not pairs:
        raise ValueError("No aligned date pairs found")

    us_dates_aligned = pd.DatetimeIndex([p[0] for p in pairs])
    jp_dates_aligned = pd.DatetimeIndex([p[1] for p in pairs])

    us_aligned = us_c2c.loc[us_dates_aligned].copy()
    us_aligned.index = range(len(us_aligned))

    jp_aligned = jp_oc.loc[jp_dates_aligned].copy()
    jp_aligned.index = range(len(jp_aligned))

    # Store the actual dates for reference
    date_index = pd.DataFrame({
        "US_DATE": us_dates_aligned.values,
        "JP_DATE": jp_dates_aligned.values,
    })

    log.info(
        "Date alignment",
        total_pairs=len(pairs),
        us_range=f"{us_dates_aligned[0].date()}..{us_dates_aligned[-1].date()}",
        jp_range=f"{jp_dates_aligned[0].date()}..{jp_dates_aligned[-1].date()}",
    )

    return us_aligned, jp_aligned, date_index


# ---------------------------------------------------------------------------
# V0 construction (paper section 3.1)
# ---------------------------------------------------------------------------
def build_V0() -> np.ndarray:
    """Construct V0 in R^(28 x 3): global, country-spread, cyclical/defensive.

    Order: first 11 = US (XLB..XLY), next 17 = JP (1617..1633).
    """
    N = N_TOTAL  # 28

    # v1: global factor (equal weight, normalised)
    v1 = np.ones(N) / np.sqrt(N)

    # v2: country spread: US = +1/sqrt(11), JP = -1/sqrt(17)
    v2_raw = np.zeros(N)
    v2_raw[:N_US] = 1.0 / np.sqrt(N_US)
    v2_raw[N_US:] = -1.0 / np.sqrt(N_JP)
    # Orthogonalise to v1
    v2_raw = v2_raw - np.dot(v2_raw, v1) * v1
    v2 = v2_raw / np.linalg.norm(v2_raw)

    # v3: cyclical/defensive
    v3_raw = np.zeros(N)
    for i, t in enumerate(US_TICKERS):
        if t in US_CYCLICAL:
            v3_raw[i] = 1.0
        elif t in US_DEFENSIVE:
            v3_raw[i] = -1.0
    for j, t in enumerate(JP_TICKERS):
        if t in JP_CYCLICAL:
            v3_raw[N_US + j] = 1.0
        elif t in JP_DEFENSIVE:
            v3_raw[N_US + j] = -1.0
    # Orthogonalise to v1, v2
    v3_raw = v3_raw - np.dot(v3_raw, v1) * v1
    v3_raw = v3_raw - np.dot(v3_raw, v2) * v2
    v3 = v3_raw / np.linalg.norm(v3_raw)

    V0 = np.column_stack([v1, v2, v3])  # 28 x 3
    log.info("V0 constructed", shape=V0.shape, orthogonality_check=float(np.max(np.abs(V0.T @ V0 - np.eye(3)))))
    return V0


# ---------------------------------------------------------------------------
# C0 construction (long-window target correlation)
# ---------------------------------------------------------------------------
def compute_C0(
    us_c2c_warm: np.ndarray,
    jp_oc_warm: np.ndarray,
    V0: np.ndarray,
) -> np.ndarray:
    """Compute C0 target correlation matrix from warm-up period.

    Following the paper:
    1. Concatenate [z_US, z_JP] into 28-dim vectors
    2. Compute full-sample correlation C_full
    3. D0 = diag(V0^T C_full V0)
    4. C_raw = V0 D0 V0^T
    5. C0 = Delta^(-1/2) C_raw Delta^(-1/2) (correlation normalisation)
    """
    # Standardise each series
    Z = np.concatenate([us_c2c_warm, jp_oc_warm], axis=1)  # T x 28
    Z = Z - Z.mean(axis=0, keepdims=True)
    std = Z.std(axis=0, keepdims=True)
    std[std == 0] = 1.0
    Z = Z / std

    C_full = (Z.T @ Z) / max(Z.shape[0] - 1, 1)  # 28 x 28

    # Paper formula
    D0 = np.diag(np.diag(V0.T @ C_full @ V0))  # 3 x 3 diagonal
    C_raw = V0 @ D0 @ V0.T  # 28 x 28
    Delta = np.diag(C_raw).copy()
    Delta[Delta <= 0] = 1e-8
    Delta_inv_sqrt = np.diag(1.0 / np.sqrt(Delta))
    C0 = Delta_inv_sqrt @ C_raw @ Delta_inv_sqrt  # 28 x 28

    log.info("C0 computed", shape=C0.shape, trace=float(np.trace(C0)))
    return C0


# ---------------------------------------------------------------------------
# Regularised PCA predictor
# ---------------------------------------------------------------------------
def compute_predictor(
    C_window: np.ndarray,
    C0: np.ndarray,
    K: int,
    lam: float,
) -> np.ndarray:
    """Compute B_t = V_JP @ V_US.T from regularised PCA.

    Returns B_t in R^(17 x 11).
    """
    C_reg = (1 - lam) * C_window + lam * C0
    C_reg = (C_reg + C_reg.T) / 2.0  # symmetrise

    vals, vecs = np.linalg.eigh(C_reg)
    # Top-K (largest eigenvalues)
    order = np.argsort(vals)[::-1][:K]
    V = vecs[:, order]  # 28 x K

    V_US = V[:N_US, :]   # 11 x K
    V_JP = V[N_US:, :]   # 17 x K
    B = V_JP @ V_US.T    # 17 x 11

    return B


# ---------------------------------------------------------------------------
# Rolling backtest
# ---------------------------------------------------------------------------
def run_backtest(
    us_c2c_aligned: pd.DataFrame,
    jp_oc_aligned: pd.DataFrame,
    date_info: pd.DataFrame,
    V0: np.ndarray,
    C0: np.ndarray,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Run rolling backtest over OOS period.

    Returns:
        ret_daily: DataFrame with daily returns for each strategy variant
        ret_weekly: DataFrame with weekly returns for weekly rebalance variants
        sig_sample: signal sample DataFrame
    """
    T = len(us_c2c_aligned)
    us_vals = us_c2c_aligned.values  # T x 11
    jp_vals = jp_oc_aligned.values   # T x 17

    # Fill NaN with 0 for computation
    us_vals = np.nan_to_num(us_vals, nan=0.0)
    jp_vals = np.nan_to_num(jp_vals, nan=0.0)

    # Determine warm-up / OOS boundaries
    jp_dates = date_info["JP_DATE"].values
    warmup_mask = jp_dates <= np.datetime64(WARMUP_END)
    oos_mask = (jp_dates >= np.datetime64(OOS_START)) & (jp_dates <= np.datetime64(OOS_END))

    warmup_end_idx = np.where(warmup_mask)[0][-1] if warmup_mask.any() else 0
    oos_indices = np.where(oos_mask)[0]

    log.info(
        "Backtest setup",
        warmup_days=int(warmup_mask.sum()),
        oos_days=len(oos_indices),
        window_L=WINDOW_L,
    )

    # Records: date -> {strategy: return}
    records = []
    signal_sample_rows = []
    sample_captured = False

    for t_idx in oos_indices:
        if t_idx < WINDOW_L:
            continue

        jp_date = pd.Timestamp(jp_dates[t_idx])
        us_date = pd.Timestamp(date_info["US_DATE"].values[t_idx])

        # Window for rolling correlation: [t_idx - L, t_idx)
        window = slice(t_idx - WINDOW_L, t_idx)
        us_win = us_vals[window]  # L x 11
        jp_win = jp_vals[window]  # L x 17

        # Build joint correlation from window
        Z_win = np.concatenate([us_win, jp_win], axis=1)  # L x 28
        Z_win = Z_win - Z_win.mean(axis=0, keepdims=True)
        std_win = Z_win.std(axis=0, keepdims=True)
        std_win[std_win == 0] = 1.0
        Z_win = Z_win / std_win
        C_win = (Z_win.T @ Z_win) / max(Z_win.shape[0] - 1, 1)

        # Regularised predictor B_t (17 x 11)
        B_reg = compute_predictor(C_win, C0, K_EIG, LAMBDA_REG)
        # Plain PCA predictor (lambda=0)
        B_plain = compute_predictor(C_win, C0, K_EIG, 0.0)

        # Standardise today's US c2c return using window stats
        us_today = us_vals[t_idx]  # 11
        us_mu = us_vals[window].mean(axis=0)
        us_sig = us_vals[window].std(axis=0)
        us_sig[us_sig == 0] = 1.0
        z_us = (us_today - us_mu) / us_sig  # standardised US c2c

        # Predicted JP oc return (standardised)
        z_pred_reg = B_reg @ z_us      # 17
        z_pred_plain = B_plain @ z_us  # 17

        # Simple US momentum baseline: rank JP sectors by the US c2c of
        # the most-correlated US sector (here we just use z_us magnitude
        # as a proxy - the "naive" version ranks JP by the US return itself).
        # Simpler: use the strongest US sector return as signal for all JP sectors.
        # But the proper baseline is: each JP sector inherits the signal from its
        # most correlated US sector. For simplicity, use B_t @ z_us with identity-like mapping.
        # Actually, simple momentum = just use z_us directly broadcast.
        # Paper says: "rank JP by US return" -> we use raw z_us as JP signal (11->17 mapping needed).
        # Simplest: signal = average US return for all, or just US market return.
        # Better: z_pred_mom[j] = sum(z_us) / 11 for all j -> same ranking -> meaningless.
        # Real simple momentum: for each JP sector, use the US sector c2c that is most
        # correlated. But that requires a mapping. Let's use: z_pred_mom = B_identity @ z_us
        # where B_identity is just the first part of B without PCA refinement.
        # Actually simplest: z_pred_mom_j = z_us[argmax correlation with j].
        # For simplicity, use average US return as signal, which means all JP get same signal.
        # That's meaningless for ranking. Instead: random signal.
        # Per paper: "MOM" = the simple momentum that correlates US sector movement to JP.
        # Let's implement: for each JP sector, the signal is the US c2c of a fixed
        # mapping based on sector similarity.
        # Given the difficulty, let's use: z_pred_mom = C_win[N_US:, :N_US] @ z_us (raw cross-corr)
        us_jp_cross = C_win[N_US:, :N_US]  # 17 x 11
        z_pred_mom = us_jp_cross @ z_us  # 17 (simple cross-correlation momentum)

        # Actual JP oc return for this day
        jp_today = jp_vals[t_idx]  # 17

        # Build per-strategy returns
        row = {"JP_DATE": jp_date, "US_DATE": us_date}

        for top_n in TOP_N_LIST:
            suffix = f"top{top_n}"

            # Regularised PCA signal
            ranking_reg = np.argsort(-z_pred_reg)
            long_idx_reg = ranking_reg[:top_n]
            ret_reg = np.mean(jp_today[long_idx_reg])
            row[f"REG_PCA_{suffix}"] = ret_reg

            # Plain PCA (lambda=0)
            ranking_plain = np.argsort(-z_pred_plain)
            long_idx_plain = ranking_plain[:top_n]
            ret_plain = np.mean(jp_today[long_idx_plain])
            row[f"PLAIN_PCA_{suffix}"] = ret_plain

            # Simple US momentum
            ranking_mom = np.argsort(-z_pred_mom)
            long_idx_mom = ranking_mom[:top_n]
            ret_mom = np.mean(jp_today[long_idx_mom])
            row[f"US_MOM_{suffix}"] = ret_mom

        # FLAT baseline
        row["FLAT"] = 0.0

        # Equal-weight all 17 (market average)
        row["EW_ALL"] = np.mean(jp_today)

        records.append(row)

        # Signal sample for a mid-OOS day
        if not sample_captured and jp_date >= pd.Timestamp("2023-07-01"):
            sample_captured = True
            for j in range(N_JP):
                signal_sample_rows.append({
                    "JP_DATE": jp_date,
                    "US_DATE": us_date,
                    "JP_TICKER": JP_TICKERS[j],
                    "SIGNAL_REG": float(z_pred_reg[j]),
                    "SIGNAL_PLAIN": float(z_pred_plain[j]),
                    "SIGNAL_MOM": float(z_pred_mom[j]),
                    "ACTUAL_OC_RET": float(jp_today[j]),
                    "RANK_REG": int(np.argsort(np.argsort(-z_pred_reg))[j]) + 1,
                })

    ret_daily = pd.DataFrame(records).set_index("JP_DATE")
    ret_daily.index.name = "DATE"
    # Drop US_DATE column for metrics
    us_dates_col = ret_daily.pop("US_DATE")

    sig_sample = pd.DataFrame(signal_sample_rows)

    # --- Weekly rebalance version ---
    # Hold positions for a week (5 trading days), rebalance on Monday (or first day of week)
    ret_weekly = build_weekly_rebalance(us_vals, jp_vals, date_info, C0, oos_indices)

    return ret_daily, ret_weekly, sig_sample


def build_weekly_rebalance(
    us_vals: np.ndarray,
    jp_vals: np.ndarray,
    date_info: pd.DataFrame,
    C0: np.ndarray,
    oos_indices: np.ndarray,
) -> pd.DataFrame:
    """Weekly rebalance: compute signal once per week, hold all week."""
    jp_dates = date_info["JP_DATE"].values
    records = []
    current_positions: dict[str, np.ndarray | None] = {f"top{n}": None for n in TOP_N_LIST}
    rebal_counter = 0

    for t_idx in oos_indices:
        if t_idx < WINDOW_L:
            continue

        jp_date = pd.Timestamp(jp_dates[t_idx])
        jp_today = jp_vals[t_idx]

        # Rebalance every 5 trading days
        if rebal_counter % 5 == 0:
            window = slice(t_idx - WINDOW_L, t_idx)
            us_win = us_vals[window]
            jp_win = jp_vals[window]

            Z_win = np.concatenate([us_win, jp_win], axis=1)
            Z_win = Z_win - Z_win.mean(axis=0, keepdims=True)
            std_win = Z_win.std(axis=0, keepdims=True)
            std_win[std_win == 0] = 1.0
            Z_win = Z_win / std_win
            C_win = (Z_win.T @ Z_win) / max(Z_win.shape[0] - 1, 1)

            B_reg = compute_predictor(C_win, C0, K_EIG, LAMBDA_REG)

            us_today = us_vals[t_idx]
            us_mu = us_vals[window].mean(axis=0)
            us_sig = us_vals[window].std(axis=0)
            us_sig[us_sig == 0] = 1.0
            z_us = (us_today - us_mu) / us_sig

            z_pred_reg = B_reg @ z_us
            ranking = np.argsort(-z_pred_reg)

            for top_n in TOP_N_LIST:
                current_positions[f"top{top_n}"] = ranking[:top_n]

        rebal_counter += 1

        row = {"DATE": jp_date}
        for top_n in TOP_N_LIST:
            suffix = f"top{top_n}"
            pos = current_positions[suffix]
            if pos is not None:
                row[f"REG_PCA_W_{suffix}"] = np.mean(jp_today[pos])
            else:
                row[f"REG_PCA_W_{suffix}"] = 0.0
        row["FLAT"] = 0.0
        records.append(row)

    ret_weekly = pd.DataFrame(records).set_index("DATE")
    return ret_weekly


# ---------------------------------------------------------------------------
# Metrics computation
# ---------------------------------------------------------------------------
def compute_metrics(ret: pd.DataFrame, cost_bps: float = ONEWAY_COST_BPS) -> pd.DataFrame:
    """Compute 3-stage metrics: gross, after-cost, after-tax."""
    rows = []
    for col in ret.columns:
        if col in ("FLAT", "EW_ALL", "US_DATE"):
            continue
        s = ret[col].dropna()
        if len(s) == 0:
            continue

        # Gross
        mu_gross = s.mean() * 252
        sig = s.std(ddof=1) * np.sqrt(252)
        sharpe_gross = mu_gross / sig if sig > 0 else np.nan
        cum_gross = (1 + s).cumprod()
        mdd_gross = ((cum_gross / cum_gross.cummax()) - 1).min()

        # After cost: daily turnover depends on strategy.
        # For long-only top-N with daily rebalance, estimate turnover.
        # Average: about 2/N_top of positions change per day (rough estimate).
        # Each change = 1 buy + 1 sell. With N_top positions, expected turnover
        # fraction per day ~ 2/N_top (but empirically higher).
        # Conservative: assume 100% turnover (all positions change daily).
        # Each position = 1/N_top weight. Cost = 2 * cost_bps / 10000 per day.
        daily_cost = 2 * cost_bps / 10_000  # round-trip cost per day
        s_net = s - daily_cost
        mu_net = s_net.mean() * 252
        sharpe_net = mu_net / sig if sig > 0 else np.nan
        cum_net = (1 + s_net).cumprod()
        mdd_net = ((cum_net / cum_net.cummax()) - 1).min()

        # After tax
        mu_tax = mu_net * (1 - TAX_RATE) if mu_net > 0 else mu_net
        sharpe_tax = mu_tax / sig if sig > 0 else np.nan

        rows.append({
            "strategy": col,
            "n_days": len(s),
            "AR_gross": mu_gross,
            "Sharpe_gross": sharpe_gross,
            "MDD_gross": mdd_gross,
            "AR_net": mu_net,
            "Sharpe_net": sharpe_net,
            "MDD_net": mdd_net,
            "AR_tax": mu_tax,
            "Sharpe_tax": sharpe_tax,
        })

    return pd.DataFrame(rows)


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------
def main() -> int:
    ts0 = datetime.now(tz=JST)
    log.info("011-4 US->JP sector lead-lag PoC start", ts=ts0.isoformat())

    # 1. Fetch data
    df_us = fetch_us_data()
    df_jp = fetch_jp_data()

    # 2. Build returns
    us_c2c = build_us_returns(df_us)
    jp_oc, jp_c2c = build_jp_returns(df_jp)

    log.info("Returns built", us_c2c_shape=us_c2c.shape, jp_oc_shape=jp_oc.shape)

    # 3. Align common dates (US day t -> JP day t+1)
    us_aligned, jp_aligned, date_info = align_common_dates(us_c2c, jp_oc)

    # 4. Build V0
    V0 = build_V0()

    # 5. Compute C0 from warm-up period
    jp_dates = date_info["JP_DATE"].values
    warmup_mask = (jp_dates >= np.datetime64(WARMUP_START)) & (jp_dates <= np.datetime64(WARMUP_END))

    us_warm = us_aligned.values[warmup_mask]
    jp_warm = jp_aligned.values[warmup_mask]
    # Fill NaN
    us_warm = np.nan_to_num(us_warm, nan=0.0)
    jp_warm = np.nan_to_num(jp_warm, nan=0.0)

    C0 = compute_C0(us_warm, jp_warm, V0)

    # 6. Run backtest
    ret_daily, ret_weekly, sig_sample = run_backtest(
        us_aligned, jp_aligned, date_info, V0, C0,
    )

    # 7. Compute metrics
    metrics_daily = compute_metrics(ret_daily)
    metrics_weekly = compute_metrics(ret_weekly, cost_bps=ONEWAY_COST_BPS)

    # Combine metrics
    metrics_all = pd.concat([metrics_daily, metrics_weekly], ignore_index=True)

    log.info("=== METRICS ===")
    print("\n" + metrics_all.to_string(index=False))

    # 8. Save outputs
    ret_daily.to_csv(OUTPUT_DIR / "returns_daily.csv", encoding="utf-8")
    ret_weekly.to_csv(OUTPUT_DIR / "returns_weekly.csv", encoding="utf-8")
    metrics_all.to_csv(OUTPUT_DIR / "metrics_summary.csv", index=False, encoding="utf-8")
    sig_sample.to_csv(OUTPUT_DIR / "signal_sample.csv", index=False, encoding="utf-8")

    log.info("Outputs saved", dir=str(OUTPUT_DIR))

    # 9. Pass/Fail judgment
    # Find best tax-adjusted Sharpe among REG_PCA variants
    reg_metrics = metrics_all[metrics_all["strategy"].str.startswith("REG_PCA")]
    if len(reg_metrics) > 0:
        best_row = reg_metrics.loc[reg_metrics["Sharpe_tax"].idxmax()]
        best_sharpe_tax = best_row["Sharpe_tax"]
        best_strategy = best_row["strategy"]
        passed = best_sharpe_tax >= 1.0
        verdict = "PASS" if passed else "FAIL"
        log.info(
            "Pass 1 judgment",
            verdict=verdict,
            best_strategy=best_strategy,
            sharpe_tax=f"{best_sharpe_tax:.4f}",
            threshold=1.0,
        )
    else:
        verdict = "FAIL"
        best_sharpe_tax = float("nan")
        log.info("No REG_PCA strategies found", verdict=verdict)

    ts1 = datetime.now(tz=JST)
    elapsed = (ts1 - ts0).total_seconds()
    log.info("Done", elapsed_s=f"{elapsed:.1f}", ts=ts1.isoformat())

    # Print summary for notification
    print(f"\n=== VERDICT: {verdict} (best tax-adjusted Sharpe = {best_sharpe_tax:.4f}) ===")

    return 0


if __name__ == "__main__":
    raise SystemExit(main())
