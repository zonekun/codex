...
PRED_COLUMNS: list[str] = [
    ...
    "is_low_base", "median_5y_op", "is_turnaround", "fy_achievement",
]
...
def compute_score(row: pd.Series) -> tuple[int, list[str]]:
    ...
    # F5: 来期ガイダンス — OP成長率とEPS成長率の大きい方を使用
    ...
    if row.get("has_stock_split"):
        _nyc_eps = None
    _candidates = [v for v in [_nyc_op, _nyc_eps] if v is not None and pd.notna(v)]
    nyc = max(_candidates) if _candidates else None
    if cur_per == "FY" and nyc is not None and pd.notna(nyc):
        if nyc > 0.10:
            _src = "EPS" if (_nyc_eps is not None and nyc == _nyc_eps and nyc != _nyc_op) else "OP"
            score += 2; reasons.append(f"来期{_src}増益 {nyc:+.1%}")
        elif nyc < -0.10:
    ...
    if (
        cur_per == "FY"
        and nyc7 is not None and pd.notna(nyc7)
    ...
    if (not row.get("has_stock_split")
        ...
        if _growth_rate is not None:
            ...
            if _peg < 0.5:
                score += 2; reasons.append(f"PEG割安 {_peg:.1f} (PER{_per:.0f}x/成長{_growth_pct:.0f}%)")
            elif _peg < 1.0:
                score += 1; reasons.append(f"PEG割安 {_peg:.1f} (PER{_per:.0f}x/成長{_growth_pct:.0f}%)")
            elif _peg > 10.0:
                score -= 3; reasons.append(f"PEG超割高 {_peg:.1f} (PER{_per:.0f}x/成長{_growth_pct:.0f}%)")
            elif _peg > 5.0:
                score -= 2; reasons.append(f"PEG割高 {_peg:.1f} (PER{_per:.0f}x/成長{_growth_pct:.0f}%)")
            elif _peg > 2.0:
                score -= 1; reasons.append(f"PEG割高 {_peg:.1f} (PER{_per:.0f}x/成長{_growth_pct:.0f}%)")
