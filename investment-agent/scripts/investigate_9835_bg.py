#!/usr/bin/env python3
"""9835 NG 調査: マッピング交叉疑い.

suggested mapping:
  ホームセンター全店 売上（前年同月比） → ホームセンター既存店 客数（前年同月比）
  ホームセンター全店 客単価（前年同月比） → ホームセンター既存店 売上（前年同月比）

全店/既存店 + 売上/客数/客単価 の交叉 → adapter 行マッピングがずれている疑い.
PDF / full_text / records を確認.
"""
...
def main() -> int:
    ...
    if adapter_path.exists():
        ...
        for k, v in adp.items():
            if k == "fields":
                ...
                for ff in v:
                    logger.info(f"    key={ff.get('key','')[:40]:40s} bc_key={ff.get('bc_key','')[:35]:35s} regex={str(ff.get('row_label_regex',''))[:80]}")
            else:
    ...
    # 最新2件 full_text で 全店/既存店/売上/客数/客単価 占有位置
    logger.info("\n--- TDnet 最新 full_text キー行 ---")
    sql = f"""
    ...
    for doc in bq.query(sql).result():
        ...
        for i, line in enumerate(lines):
            if any(kw in line for kw in ["全店", "既存店", "客数", "客単価", "売上", "ホームセンター"]):
                logger.info(f"  L{i}: {line[:200]}")
                if i > 40:
                    break
