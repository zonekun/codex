"""Mistral Medium 3 PoC: 配当変更(増減配) 検出テスト.

Vertex AI Model Garden MaaS 経由で Mistral Medium 3 (123B dense) を呼び出し、
既存の233件の配当変更サンプル（Geminiが検出した決算短信）での検出率を測定する。

比較対象:
- Gemini (正解): 233/233 (100%)
- Gemma 26B MaaS: 155/233 (66.5%)
- Mistral Medium 3: ??/233 ← 今回テスト

クォータ制約:
- 9 QPM / 31,500 TPM
- 30K文字入力だと 1リクエスト/分の制約
- 233件で約4時間

Usage:
    PYTHONUTF8=1 python scripts/poc_mistral_haito.py
    PYTHONUTF8=1 python scripts/poc_mistral_haito.py --limit 3  # 接続確認
"""
...
OUT_CACHE = CACHE_DIR / "mistral_medium3_haito.json"

# Vertex AI MaaS (Mistral native rawPredict エンドポイント)
MISTRAL_MODEL = "mistral-medium-3"
LOCATION = "us-central1"

# レート制御: 31,500 TPM / 30K tokens ≒ 1 req/min
REQUEST_INTERVAL_SEC = 65
...
def call_mistral(access_token: str, prompt: str, max_retries: int = 3) -> dict | None:
    """Mistral Medium 3 (Vertex AI MaaS) を呼び出す."""
    import requests

    # Vertex AI Mistral native rawPredict エンドポイント
    url = (
        f"https://{LOCATION}-aiplatform.googleapis.com/v1/projects/"
        f"{GCP_PROJECT}/locations/{LOCATION}/publishers/mistralai/models/"
        f"{MISTRAL_MODEL}:rawPredict"
    )
    headers = {
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.warning("mistral_call_error", error=str(e)[:200], attempt=attempt)
...
def run_poc(limit: int | None = None) -> None:
    ...
    total_estimated_min = len(haito_samples) * REQUEST_INTERVAL_SEC / 60
    logger.info(
        "estimated_runtime",
        samples=len(haito_samples),
        interval_sec=REQUEST_INTERVAL_SEC,
        estimated_minutes=f"{total_estimated_min:.1f}",
    )
    ...
    for i, sample in enumerate(haito_samples):
        ...
        result = call_mistral(access_token, prompt)
        ...
        results.append({
            ...
            "mistral_medium3": haito,
            "mistral_reason": reason,
            "status": status,
        })

        # 進捗表示: 3件毎に出す（長時間ジョブ用に細かく）
        if (i + 1) % 3 == 0 or i == len(haito_samples) - 1:
            ...
            # 途中結果を随時保存（ジョブ中断時の安全策）
            CACHE_DIR.mkdir(parents=True, exist_ok=True)
            with open(OUT_CACHE, "w", encoding="utf-8") as f:
                json.dump(results, f, ensure_ascii=False, indent=2)

        # レート制御: 次のリクエストまで REQUEST_INTERVAL_SEC 秒確保
        elapsed = time.time() - call_start
        sleep_sec = max(0, REQUEST_INTERVAL_SEC - elapsed)
    ...
    mistral_detected = sum(1 for r in results if r["mistral_medium3"] is True)

    print("\n" + "=" * 60)
    print("Mistral Medium 3 配当変更(増減配) 検出 PoC 結果")
    print("=" * 60)
    ...
    print(f"Mistral Medium 3  : {mistral_detected} / {len(results)} "
          f"({mistral_detected / len(results):.1%})")
    print("=" * 60)
