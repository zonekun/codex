"""gemma3:12b モデルを GCS にアップロードする初回セットアップスクリプト.
...
実行方法:
    PYTHONUTF8=1 python scripts/setup_gemma3_gcs.py

アップロード先:
    gs://stock_data_1930932/test/ollama-models-gemma3/
    → Cloud Run Job が /mnt/gemma3 に FUSE マウント

Ollama ディレクトリ構造（マウント後）:
    /mnt/gemma3/test/ollama-models-gemma3/
    └── models/
        ├── manifests/registry.ollama.ai/library/gemma3/12b
...
GCS_DEST = "gs://stock_data_1930932/test/ollama-models-gemma3"
...
def pull_model(model: str = MODEL_NAME) -> None:
    ...
    Args:
        model: モデル名（例: gemma3:12b）。
    """
    logger.info("model_pull_start", model=model,
                message="約7.7GB のダウンロードが開始されます。数分〜数十分かかります")
...
def upload_to_gcs(models_dir: Path, gcs_dest: str) -> None:
    ...
    logger.info("gcs_upload_start", src=src, dest=gcs_dest,
                message="モデルファイルを GCS にアップロードします（約7.7GB）")
...
def main() -> None:
    ...
    logger.info("setup_complete",
                message=f"モデルを {GCS_DEST} にアップロードしました。"
                        "次のステップ: cloudbuild でイメージビルド → jobs create でジョブ作成")
