...
| 5 | 翌期見通し | >+10% (OP/EPS大きい方) | +2 | <-10% / 非開示(cap≥3000億)→-1 | -2/-1 | FYのみ | J-Quants NxFOP vs OP + NxFEPS vs EPS。低ベース時は5年中央値比較。F14発火時はEPS無効化 |
| 6 | 出尽くしリスク | — | — | 進捗>90% + 据え置き | -2 | 3Qのみ | F1+F2の組合せ |
| 7 | 成長加速/減速 | gap>+20pt | +1 | gap<-20pt | -1 | FYのみ | BQ 過去FY YoY OP median。低ベース時は中央値ベースのnext_year_op_changeで自動調整 |
...
| 12 | PER割安度(PEG) | PEG<0.5→+2, <1.0→+1 | +1/+2 | PEG>2.0→-1, >5.0→-2, >10.0→-3 | -1/-2/-3 | **全Q** | FY: 来期OP成長率、非FY: YoY OP成長率で代替 （※F14発火時は無効化） |
| 13 | QoQ OP急変 | 前Q比>+50% | +1 | 前Q比<-50% | -2 | 全Q | BQ v_fin_summary_actual_for_q_on_q |
| 14 | 株式分割 | TDnet TITLE "株式分割" | +1 | — | — | 全Q | yanoshin / TDnet HTML / BQ |
| 15 | 黒字転換サプライズ | 前年同期赤字→今期黒字 | +2 (YoYあり) / +1 | — | — | 全Q | BQ v_fin_summary_actual_for_q_on_q (prev_year_op < 0 & cur_standalone_op > 0) |
| 16 | FY予想未達ペナルティ | — | — | 達成率<80%→-1, <65%→-2 | -1/-2 | FYのみ | J-Quants OP÷FOP |

> **F14注記**: F14発火時はF11(増配/減配)・F12(PEG)・F5(EPS成長率)を無効化（暫定措置、分割比率未調整のため）
> **低ベースフィルタ**: FY時、今期OPが5年中央値OPの50%未満→低ベースフラグ。F5の比較対象を「今期→来期」から「5年中央値→来期」に差替え
...
| 12 | F3 YoY OP 廃止 | 保留 | 全般 | EDA待ち | F13(QoQ)との相関確認。除去で精度変化あるか |
